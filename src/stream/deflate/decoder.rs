...
pub struct DeflateDecoder<S: Stream<Item = Result<Bytes>>> {
    #[pin]
    inner: crate::stream::generic::Decoder<S, crate::codec::DeflateDecoder>,
}
...
impl<S: Stream<Item = Result<Bytes>>> DeflateDecoder<S> {
    ...
    pub fn new(stream: S) -> Self {
        Self {
            inner: crate::stream::generic::Decoder::new(
                stream,
...
fn _assert() {
    crate::util::_assert_send::<DeflateDecoder<Pin<Box<dyn Stream<Item = Result<Bytes>> + Send>>>>(
    );
    crate::util::_assert_sync::<DeflateDecoder<Pin<Box<dyn Stream<Item = Result<Bytes>> + Sync>>>>(
    );
}
